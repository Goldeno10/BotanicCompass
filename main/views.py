            ...
            if field_name:
                try:
                    ...
                    if search_by == 'scientific_name':
                        plant_data = PlantData.objects.filter(scientific_name__icontains=field_name).first()

                    elif search_by == 'hausa_name':
                        plant_data = PlantData.objects.filter(hausa_name__name__icontains=field_name).first()

                    elif search_by == 'common_name':
                        plant_data = PlantData.objects.filter(common_name__name__icontains=field_name).first()
